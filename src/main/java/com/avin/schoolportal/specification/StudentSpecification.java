...
public class StudentSpecification implements Specification<Student> {
    ...
    public Predicate toPredicate(Root<Student> root, CriteriaQuery<?> criteriaQuery, CriteriaBuilder criteriaBuilder) {
        ...
        if (clazz.equals(Student.class)) {
            root.fetch(Student_.user).fetch(User_.person);
        ...
        Path<Person> personPath = root.get(Student_.user).get(User_.person);
        ...
        if (params.get("filter.user.person.fatherName") != null)
            predicates.add(criteriaBuilder.like(personPath.get(Person_.fatherName), "%" + params.get("filter.user.person.fatherName") + "%"));
